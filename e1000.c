...
#define E1000_FLAG_VET (1 << E1000_FLAG_VET_BIT)
    ...
    struct {
        /* Global RDMA state */
        uint32_t ctrl;           /* RDMA control register */
        uint32_t status;         /* RDMA status register */
        uint64_t mr_table_ptr;   /* Guest physical address of MR table */
        uint32_t mr_table_len;   /* Number of MR entries */
        
        /* Per-QP state */
        struct {
            uint64_t sq_base;    /* Send Queue base address */
            uint32_t sq_size;    /* Send Queue size */
            uint32_t sq_head;    /* Send Queue head pointer */
            uint32_t sq_tail;    /* Send Queue tail pointer */
            uint64_t cq_base;    /* Completion Queue base */
            uint32_t cq_size;    /* Completion Queue size */
            uint32_t cq_head;    /* Completion Queue head */
            uint32_t cq_tail;    /* Completion Queue tail */
            bool valid;          /* QP is configured */
        } qp[E1000_QP_COUNT];
        
        /* RDMA packet processing */
        QEMUTimer *work_timer;   /* Timer for processing work */
    } rdma;
};
...
/* ============================================
 * RDMA Data Structures (Guest Memory Format)
 * ============================================ */

/* Work Request (what user posts to Send Queue) */
struct rdma_work_request {
    uint64_t wr_id;          /* User-provided identifier */
    uint8_t  opcode;         /* RDMA_OP_WRITE, etc. */
    uint8_t  flags;          /* Signaled, etc. */
    uint16_t reserved;
    uint32_t local_mr_id;    /* Source memory region */
    uint64_t local_offset;   /* Offset within local MR */
    uint32_t remote_mr_id;   /* Destination memory region */
    uint64_t remote_addr;    /* Remote address */
    uint32_t remote_key;     /* Remote key for validation */
    uint32_t length;         /* Transfer length */
} QEMU_PACKED;

/* Completion Queue Entry (what NIC posts) */
struct rdma_completion {
    uint64_t wr_id;          /* Matches work_request.wr_id */
    uint32_t byte_len;       /* Bytes transferred */
    uint8_t  status;         /* Success/error code */
    uint8_t  opcode;         /* Echo of operation */
    uint16_t reserved;
} QEMU_PACKED;

/* Memory Region (in guest kernel memory) */
struct rdma_mr {
    uint32_t id;             /* MR identifier */
    ...
    uint32_t valid;          /* 1 = active */
} QEMU_PACKED;

/* RDMA Packet Header (on wire) */
struct rdma_packet_header {
    uint16_t ethertype;      /* ETH_P_RDMA */
    uint8_t  opcode;         /* RDMA_OP_* */
    uint8_t  version;        /* Protocol version = 1 */
    uint32_t dest_qp;        /* Destination QP number */
    uint32_t remote_key;     /* For validation */
    uint64_t remote_addr;    /* Where to write/read */
    uint32_t length;         /* Payload length */
    uint64_t wr_id;          /* For completion matching */
    uint32_t src_qp;         /* Source QP number */
    uint32_t padding;
} QEMU_PACKED;
...
/* ============================================
 * RDMA Function Forward Declarations
 * ============================================ */
static int e1000_rdma_dma_write(E1000State *s, hwaddr addr,
                                const void *buf, size_t len);
static bool e1000_rdma_validate_mr(E1000State *s, uint32_t rkey,
                                  uint64_t addr, uint32_t length,
                                  uint32_t required_access);
static void e1000_rdma_post_completion(E1000State *s, uint32_t qp_num,
                                       uint64_t wr_id, uint8_t status,
                                       uint8_t opcode, uint32_t byte_len);

/* ============================================
 * RDMA Packet Reception
 * ============================================ */

/* Check if packet is RDMA packet */
static bool e1000_is_rdma_packet(const uint8_t *buf, size_t size)
{
    if (size < 14 + sizeof(struct rdma_packet_header)) {
        return false;
    }

    /* Check ethertype */
    uint16_t ethertype = ntohs(*(uint16_t *)(buf + 12));
    return (ethertype == ETH_P_RDMA);
}

/* Handle incoming RDMA packet */
static void e1000_handle_rdma_packet(E1000State *s, const uint8_t *buf,
                                     size_t size)
{
    const struct rdma_packet_header *hdr;
    ...
    /* Parse packet */
    hdr = (const struct rdma_packet_header *)(buf + 14);
    payload = buf + 14 + sizeof(struct rdma_packet_header);
    payload_len = size - 14 - sizeof(struct rdma_packet_header);

    /* Process based on opcode */
    switch (hdr->opcode) {
    case RDMA_OP_WRITE:
        /* Validate destination address and key */
        if (!e1000_rdma_validate_mr(s, hdr->remote_key, hdr->remote_addr,
                                   hdr->length, 0x08)) {  /* WRITE permission */
            qemu_log_mask(LOG_GUEST_ERROR,
                         "e1000_rdma: WRITE validation failed\n");
            return;
        }

        /* DMA write to destination */
        if (e1000_rdma_dma_write(s, hdr->remote_addr, payload,
                                 hdr->length) == 0) {
            /* Post completion to destination QP */
            uint32_t dest_qp = hdr->dest_qp;
            if (dest_qp < E1000_QP_COUNT && s->rdma.qp[dest_qp].valid) {
                e1000_rdma_post_completion(s, dest_qp, hdr->wr_id,
                                          RDMA_WC_SUCCESS,
                                          RDMA_OP_WRITE, hdr->length);
            }
        }
        break;

    case RDMA_OP_READ:
        /* TODO: Handle RDMA_READ request - send back data */
        break;

    case RDMA_OP_SEND:
        /* TODO: Handle RDMA_SEND - match with posted receive */
        break;

    case RDMA_OP_READ_RESP:
        /* TODO: Handle RDMA_READ response */
        break;

    default:
        qemu_log_mask(LOG_GUEST_ERROR,
                     "e1000_rdma: Unknown opcode in received packet: %d\n",
                     hdr->opcode);
    }
}
...
e1000_receive_iov(NetClientState *nc, const struct iovec *iov, int iovcnt)
{
    ...
    /* ============================================
     * Check for RDMA packet first
     * ============================================ */
    if ((s->rdma.ctrl & E1000_RDMA_CTRL_ENABLE) && size > 0) {
        /* Flatten iovec for easier processing */
        uint8_t *buf = g_malloc(size);
        if (buf) {
            iov_to_buf(iov, iovcnt, 0, buf, size);

            /* Check if RDMA packet */
            if (e1000_is_rdma_packet(buf, size)) {
                /* Handle RDMA packet */
                e1000_handle_rdma_packet(s, buf, size);
                g_free(buf);
    ...
    /* Continue with normal E1000 receive path */
...
/* Read guest memory (DMA read) */
static int e1000_rdma_dma_read(E1000State *s, hwaddr addr,
                               void *buf, size_t len)
{
    if (pci_dma_read(&s->parent_obj, addr, buf, len) != 0) {
        qemu_log_mask(LOG_GUEST_ERROR,
                     "e1000_rdma: DMA read failed at 0x%lx\n", (unsigned long)addr);
        return -1;
    }
    return 0;
}

/* Write guest memory (DMA write) */
static int e1000_rdma_dma_write(E1000State *s, hwaddr addr,
                                const void *buf, size_t len)
{
    if (pci_dma_write(&s->parent_obj, addr, buf, len) != 0) {
        qemu_log_mask(LOG_GUEST_ERROR,
                     "e1000_rdma: DMA write failed at 0x%lx\n", (unsigned long)addr);
        return -1;
    }
    return 0;
}
                                  ...
                                  uint32_t required_access)
{
    ...
    /* Sanity check table pointer */
    if (s->rdma.mr_table_ptr == 0 || s->rdma.mr_table_len == 0) {
        return false;
    }

    /* Search MR table in guest memory */
    for (uint32_t i = 0; i < s->rdma.mr_table_len; i++) {
        hwaddr mr_addr = s->rdma.mr_table_ptr + i * sizeof(struct rdma_mr);

        if (e1000_rdma_dma_read(s, mr_addr, &mr, sizeof(mr)) != 0) {
            return false;
        }
        ...
        /* Check if rkey matches */
        if (mr.rkey == rkey) {
            /* Check address range */
            if (addr >= mr.paddr && addr + length <= mr.paddr + mr.length) {
                /* Check permissions */
                if ((mr.access_flags & required_access) == required_access) {
                    return true;
                }
            }
        }
                                       ...
                                       uint8_t opcode, uint32_t byte_len)
{
    ...
    /* Calculate CQ entry address */
    uint32_t tail = s->rdma.qp[qp_num].cq_tail;
    uint32_t size = s->rdma.qp[qp_num].cq_size;
    hwaddr cq_addr = s->rdma.qp[qp_num].cq_base +
                     (tail % size) * sizeof(struct rdma_completion);

    /* Write completion to guest memory */
    if (e1000_rdma_dma_write(s, cq_addr, &comp, sizeof(comp)) == 0) {
        /* Advance tail pointer */
        s->rdma.qp[qp_num].cq_tail = (tail + 1) % size;
    }
}

/* Reset RDMA state */
static void e1000_rdma_reset(E1000State *s)
{
    s->rdma.ctrl = 0;
    s->rdma.status = 0;
    s->rdma.mr_table_ptr = 0;
    s->rdma.mr_table_len = 0;

    for (int i = 0; i < E1000_QP_COUNT; i++) {
        memset(&s->rdma.qp[i], 0, sizeof(s->rdma.qp[i]));
    }
}

/* Process RDMA WRITE operation */
static void e1000_rdma_do_write(E1000State *s, uint32_t qp_num,
                                struct rdma_work_request *wr)
{
    uint8_t *buffer;
    struct rdma_packet_header hdr;
    uint8_t status = RDMA_WC_SUCCESS;

    /* Allocate buffer for payload */
    buffer = g_malloc(wr->length);
    if (!buffer) {
        status = RDMA_WC_LOC_LEN_ERR;
        goto post_completion;
    }

    /* Step 1: DMA read source data from guest */
    if (e1000_rdma_dma_read(s, wr->local_offset, buffer, wr->length) != 0) {
        status = RDMA_WC_LOC_PROT_ERR;
        goto cleanup;
    }

    /* Step 2: Build RDMA packet header */
    hdr.ethertype = htons(ETH_P_RDMA);
    hdr.opcode = RDMA_OP_WRITE;
    hdr.version = 1;
    hdr.dest_qp = wr->remote_mr_id;
    hdr.remote_key = wr->remote_key;
    hdr.remote_addr = wr->remote_addr;
    hdr.length = wr->length;
    hdr.wr_id = wr->wr_id;
    hdr.src_qp = qp_num;

    /* Step 3: Send packet (using existing E1000 transmit) */
    size_t total_len = sizeof(hdr) + wr->length;
    uint8_t *packet = g_malloc(total_len + 14);

    if (packet) {
        /* Ethernet header (broadcast for simplicity) */
        memset(packet, 0xff, 6);      /* Dest MAC: broadcast */
        memset(packet + 6, 0, 6);     /* Src MAC: zeros */
        *(uint16_t *)(packet + 12) = htons(ETH_P_RDMA);

        /* RDMA header and payload */
        memcpy(packet + 14, &hdr, sizeof(hdr));
        memcpy(packet + 14 + sizeof(hdr), buffer, wr->length);

        /* Send via QEMU network backend */
        qemu_send_packet(qemu_get_queue(s->nic), packet, total_len + 14);
...
post_completion:
    /* Post completion */
    if (wr->flags & 0x01) {  /* Signaled */
        e1000_rdma_post_completion(s, qp_num, wr->wr_id, status,
                                  RDMA_OP_WRITE, wr->length);
    }
}

/* Process Send Queue for a QP */
static void e1000_rdma_process_sq(E1000State *s, uint32_t qp_num)
{
    struct rdma_work_request wr;
    ...
    while (s->rdma.qp[qp_num].sq_head != s->rdma.qp[qp_num].sq_tail) {
        uint32_t head = s->rdma.qp[qp_num].sq_head;
        uint32_t size = s->rdma.qp[qp_num].sq_size;

        /* Read work request from guest memory */
        hwaddr wr_addr = s->rdma.qp[qp_num].sq_base +
                         (head % size) * sizeof(struct rdma_work_request);

        if (e1000_rdma_dma_read(s, wr_addr, &wr, sizeof(wr)) != 0) {
            qemu_log_mask(LOG_GUEST_ERROR,
                         "e1000_rdma: Failed to read WR at qp=%d head=%d\n",
                         qp_num, head);
            break;
        }
        ...
        switch (wr.opcode) {
        case RDMA_OP_WRITE:
            e1000_rdma_do_write(s, qp_num, &wr);
            break;

        case RDMA_OP_READ:
            /* TODO: Implement RDMA_READ */
            e1000_rdma_post_completion(s, qp_num, wr.wr_id,
                                      RDMA_WC_LOC_PROT_ERR,
                                      RDMA_OP_READ, 0);
            break;

        case RDMA_OP_SEND:
            /* TODO: Implement RDMA_SEND */
            e1000_rdma_post_completion(s, qp_num, wr.wr_id,
                                      RDMA_WC_LOC_PROT_ERR,
                                      RDMA_OP_SEND, 0);
            break;
        ...
        default:
            qemu_log_mask(LOG_GUEST_ERROR,
                         "e1000_rdma: Unknown opcode %d\n", wr.opcode);
        }

        /* Advance head pointer */
        s->rdma.qp[qp_num].sq_head = (head + 1) % size;
    }
}
                 ...
                 unsigned size)
{
    ...
    /* ============================================
     * RDMA Register Handling
     * ============================================ */

    /* RDMA Control Registers */
    if (addr == E1000_RDMA_CTRL) {
        s->rdma.ctrl = val;

        if (val & E1000_RDMA_CTRL_ENABLE) {
            /* Initialize RDMA subsystem */
            s->rdma.status |= E1000_RDMA_STATUS_READY;
        }

        if (val & E1000_RDMA_CTRL_RESET) {
            /* Reset RDMA state */
            e1000_rdma_reset(s);
        }
        return;
    ...
    if (addr == E1000_MR_TABLE_PTR) {
        s->rdma.mr_table_ptr = val;
        return;
    }

    if (addr == E1000_MR_TABLE_LEN) {
        s->rdma.mr_table_len = val;
        return;
    }

    /* Queue Pair Registers */
    if (addr >= E1000_QP_BASE &&
        addr < E1000_QP_BASE + E1000_QP_COUNT * E1000_QP_STRIDE) {

        uint32_t qp_num = (addr - E1000_QP_BASE) / E1000_QP_STRIDE;
        uint32_t qp_offset = (addr - E1000_QP_BASE) % E1000_QP_STRIDE;

        switch (qp_offset) {
        case E1000_QP_SQ_BASE:
            s->rdma.qp[qp_num].sq_base = val;
            break;

        case E1000_QP_SQ_SIZE:
            s->rdma.qp[qp_num].sq_size = val;
            break;

        case E1000_QP_SQ_TAIL:
            /* Doorbell! Process work queue */
            s->rdma.qp[qp_num].sq_tail = val;
            e1000_rdma_process_sq(s, qp_num);
            break;

        case E1000_QP_CQ_BASE:
            s->rdma.qp[qp_num].cq_base = val;
            break;

        case E1000_QP_CQ_SIZE:
            s->rdma.qp[qp_num].cq_size = val;
            s->rdma.qp[qp_num].valid = true;
            break;

        case E1000_QP_CQ_HEAD:
            /* Guest consumed completions */
            s->rdma.qp[qp_num].cq_head = val;
            break;

        default:
            /* Read-only register or unknown */
            break;
        }
        return;
    }

    /* Existing E1000 register handling */
...
e1000_mmio_read(void *opaque, hwaddr addr, unsigned size)
{
    ...
    if (addr == E1000_RDMA_STATUS) {
        return s->rdma.status;
    }

    if (addr == E1000_MR_TABLE_PTR) {
        return s->rdma.mr_table_ptr;
    }

    if (addr == E1000_MR_TABLE_LEN) {
        return s->rdma.mr_table_len;
    }

    /* Queue Pair Register Reads */
    if (addr >= E1000_QP_BASE &&
        addr < E1000_QP_BASE + E1000_QP_COUNT * E1000_QP_STRIDE) {
        ...
        switch (qp_offset) {
        case E1000_QP_SQ_BASE:
            return s->rdma.qp[qp_num].sq_base;
        case E1000_QP_SQ_SIZE:
            return s->rdma.qp[qp_num].sq_size;
        case E1000_QP_SQ_HEAD:
            return s->rdma.qp[qp_num].sq_head;
        case E1000_QP_SQ_TAIL:
            return s->rdma.qp[qp_num].sq_tail;
        case E1000_QP_CQ_BASE:
            return s->rdma.qp[qp_num].cq_base;
        case E1000_QP_CQ_SIZE:
            return s->rdma.qp[qp_num].cq_size;
        case E1000_QP_CQ_HEAD:
            return s->rdma.qp[qp_num].cq_head;
        case E1000_QP_CQ_TAIL:
            return s->rdma.qp[qp_num].cq_tail;
        default:
            return 0;
        }
    }
